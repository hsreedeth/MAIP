...
## **External validation and prognosis**

## Survival differences across clusters

Kaplan–Meier curves for all-cause mortality differed significantly between clusters within each multimorbidity stratum (overall log-rank p = 5.8×10⁻²⁷ in High_MM, 8.0×10⁻⁴⁶ in Mid_MM and 7.5×10⁻⁸² in Low_MM). This indicates that, even within bands of similar baseline multimorbidity burden, the derived phenotypes capture substantial variation in mortality risk.

## Cox models adjusted for acute severity

In Cox proportional hazards models fitted separately within each stratum and adjusted for the Acute Physiology Score (APS), both APS and cluster membership were strongly associated with mortality.

**_High multimorbidity stratum (High_MM)_**
A one-standard-deviation increase in APS was associated with a 55% higher hazard of death (HR 1.55, 95% CI 1.37–1.75; p ≈ 2×10⁻¹²). Compared with the reference cluster in this stratum, the two alternative High_MM phenotypes had substantially lower mortality: HR 0.68 (95% CI 0.55–0.84; p = 2.5×10⁻⁴) and HR 0.65 (95% CI 0.53–0.81; p = 9.0×10⁻⁵), corresponding to approximately 30–35% relative risk reductions after adjusting for APS. This suggests that, among patients with high multimorbidity, the reference phenotype represents the worst-prognosis group, whereas the other high-burden phenotypes are comparatively less lethal despite similar comorbidity counts.

**_Mid multimorbidity stratum (Mid_MM)_**
APS remained a strong predictor (HR 1.55 per standard deviation, 95% CI 1.46–1.64; p ≈ 1×10⁻⁴⁵). Cluster membership was also independently associated with mortality: two Mid_MM phenotypes showed elevated risk compared with the reference cluster, with HR 1.33 (95% CI 1.19–1.48; p ≈ 2×10⁻⁷) and HR 1.19 (95% CI 1.08–1.32; p = 8.0×10⁻⁴). Thus, even within an intermediate comorbidity band, phenotypes differ by roughly 20–30% in adjusted mortality risk.

**_Low multimorbidity stratum (Low_MM)_**
In patients with lower multimorbidity burden, APS was again strongly associated with mortality (HR 1.72 per standard deviation, 95% CI 1.63–1.81; p < 1×10⁻⁹³). Cluster membership added substantial prognostic information: one Low_MM phenotype had almost double the hazard of death relative to the reference cluster (HR 1.72, 95% CI 1.48–2.01; p ≈ 2×10⁻¹²), whereas another had approximately half the hazard (HR 0.46, 95% CI 0.41–0.51; p ≈ 8×10⁻⁵¹). This indicates that, among patients with relatively few chronic conditions, the SNF-lite phenotypes separate a high-risk acute organ-failure phenotype from a low-risk, low-acuity group.
...
km_paths <- c(
  "reports/figures/km_High_MM.png",
  "reports/figures/km_Mid_MM.png",
  "reports/figures/km_Low_MM.png"
)
...
image_write(km_all, path = "reports/figures/km_all_strata.png")

# include in the document
include_graphics("reports/figures/km_all_strata.png")
```
...
Length of stay and hospital cost also differed markedly between clusters. Kruskal–Wallis tests within each stratum showed highly significant heterogeneity in ICU/hospital length of stay (slos) and total hospital cost (totmcst):

`r margin_note("Table X. Kruskal–Wallis tests for length of stay and total hospital cost across clusters, by multimorbidity stratum.")`

| Stratum  | p-value for LOS (`slos`) | p-value for total cost (`totmcst`) |
|----------|--------------------------|------------------------------------|
| High MM  | 1.34 × 10^-17            | 1.30 × 10^-50                      |
| Mid MM   | 8.02 × 10^-76            | 1.03 × 10^-146                     |
| Low MM   | 2.22 × 10^-157           | 1.49 × 10^-157                     |
...
cf_paths <- c(
  "reports/surrogate_high/figures/confusion_matrix.png",
  "reports/surrogate_mid/figures/confusion_matrix.png",
  "reports/surrogate_low/figures/confusion_matrix.png"
)
...
# (optional) write to disk if you want to reuse it
image_write(km_all, path = "reports/figures/cf_all_strata.png")

# include in the document
include_graphics("reports/figures/cf_all_strata.png")
```
